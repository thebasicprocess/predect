...
export function ResultsView() {
  ...
  const handleExport = () => {
    ...
    lines.push("");
    lines.push(`**Confidence:** ${Math.round(report.confidence.score * 100)}%`);
    if (Array.isArray(report.confidence.band) && report.confidence.band.length === 2) {
      lines.push(`**Range:** ${Math.round(report.confidence.band[0] * 100)}%–${Math.round(report.confidence.band[1] * 100)}%`);
    }
    lines.push("");
    lines.push("## Verdict");
    ...
    lines.push(`- **Base (${Math.round(report.scenarios.base.probability * 100)}%):** ${report.scenarios.base.description}`);
    lines.push(`- **Bull (${Math.round(report.scenarios.bull.probability * 100)}%):** ${report.scenarios.bull.description}`);
    lines.push(`- **Bear (${Math.round(report.scenarios.bear.probability * 100)}%):** ${report.scenarios.bear.description}`);
    lines.push("");
    if (report.keyDrivers?.length > 0) {
      lines.push("## Key Drivers");
      report.keyDrivers.forEach((d) => lines.push(`- ${d}`));
      lines.push("");
    }
    if (report.riskFactors?.length > 0) {
      lines.push("## Risk Factors");
      report.riskFactors.forEach((r) => lines.push(`- ${r}`));
      lines.push("");
    }
    if (report.timelineOutlook?.length > 0) {
      lines.push("## Timeline Outlook");
      report.timelineOutlook.forEach((t) => lines.push(`- **${t.period}:** ${t.outlook}`));
      lines.push("");
    }
    if (report.dominantNarratives?.length > 0) {
      lines.push("## Dominant Narratives");
      report.dominantNarratives.forEach((n, i) => lines.push(`${i + 1}. ${n}`));
      lines.push("");
    }
    lines.push(`---`);
    lines.push(`*Generated by PREDECT on ${new Date().toLocaleDateString()}*`);

    const blob = new Blob([lines.join("\n")], { type: "text/markdown" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = `prediction-${new Date().toISOString().slice(0, 10)}.md`;
    a.click();
